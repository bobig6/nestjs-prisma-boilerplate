...
describe('UserController', () => {
  ...
  describe('register', () => {
    it('should register a new user', async () => {
      ...
      userService.create.mockResolvedValue({ user_id: 1, username: 'test', password_hash: 'hash', role: Role.NONE, created_at: new Date(), updated_at: new Date() });

      const result = await controller.register(dto);

      expect(userService.create).toHaveBeenCalledWith(dto);
      expect(result).toEqual({ user_id: 1, username: 'test', password_hash: 'hash', role: Role.NONE, created_at: expect.any(Date), updated_at: expect.any(Date) });
    });
    ...
    it('should throw an error if registration fails', async () => {
      ...
      await expect(controller.register(dto)).rejects.toThrow('Creating User Failed');
    });
  });
  ...
  describe('getAll', () => {
    it('should return all users', async () => {
      userService.getAll.mockResolvedValue([{ user_id: 1, username: 'test', password_hash: 'hash', role: Role.NONE, created_at: new Date(), updated_at: new Date() }]);
  ...
  describe('changeRole', () => {
    it('should change the role of a user', async () => {
      ...
      userService.changeRole.mockResolvedValue({ user_id: 1, username: 'test', password_hash: 'hash', role: Role.ADMIN, created_at: new Date(), updated_at: new Date() });

      const result = await controller.changeRole(dto);

      expect(userService.changeRole).toHaveBeenCalledWith(1, Role.ADMIN);
      expect(result).toEqual({ user_id: 1, username: 'test', password_hash: 'hash', role: Role.ADMIN, created_at: expect.any(Date), updated_at: expect.any(Date) });
    });
